(() => {
  ...
  const hiddenInput = document.getElementById('blogContent');
  const editorRoot = document.getElementById('blogContentEditor');
  const toolbarRoot = document.getElementById('blogContentToolbar');
  const mediaInput = document.getElementById('blogContentMediaInput');
  const mediaTokensInput = document.getElementById('blogContentMediaTokens');
  ...
  BlogVideoBlot.tagName = 'video';

  window.Quill.register(BlogVideoBlot);
  ...
  const quill = new window.Quill(editorRoot, {
    theme: 'snow',
    placeholder: editorRoot.dataset.placeholder || 'Nhập nội dung blog...',
    modules: {
      toolbar: {
    ...
    reader.onload = () => {
      ...
      requestAnimationFrame(() => {
        ...
        if (imgNode) {
          ...
          imgNode.alt = file.name || 'Ảnh nội dung blog';
        }
        syncEditorValue();
      });
    };
  ...
  function insertVideo(file) {
    ...
    quill.insertEmbed(range.index, 'blogVideo', { src: previewUrl, token }, 'user');
  ...
  function serializeEditorHtml() {
    ...
    clone.querySelectorAll('[data-upload-token]').forEach((node) => {
      const token = String(node.dataset.uploadToken || '').trim();
      if (!token) return;
      node.setAttribute('src', `${BLOG_MEDIA_TOKEN_PREFIX}${token}`);
  ...
  function emitPreviewHtml() {
    hiddenInput.dataset.previewHtml = quill.root.innerHTML.trim();
    hiddenInput.dispatchEvent(new CustomEvent('richtext:change', {
      detail: { html: hiddenInput.dataset.previewHtml || '' }
    }));
  }
  ...
  window.blogContentEditorApi = {
    reset() {
      quill.setContents([]);
      mediaRegistry.clear();
      mediaTokensInput.value = '[]';
      hiddenInput.value = '';
      hiddenInput.dataset.previewHtml = '';
      mediaInput.value = '';
      emitPreviewHtml();
    }
  };
