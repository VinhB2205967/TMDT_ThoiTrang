(() => {
    ...
    const hiddenInput = document.getElementById('lookbookDescriptionInput');
    const editorRoot = document.getElementById('lookbookDescriptionEditor');
    const toolbarRoot = document.getElementById('lookbookDescriptionToolbar');
    const mediaInput = document.getElementById('lookbookDescriptionMediaInput');
    const mediaTokensInput = document.getElementById('lookbookDescriptionMediaTokens');
    ...
    class LookbookVideoBlot extends BlockEmbed {
        static create(value) {
    ...
    LookbookVideoBlot.blotName = 'lookbookVideo';
    LookbookVideoBlot.tagName = 'video';

    window.Quill.register(LookbookVideoBlot);
    ...
    const quill = new window.Quill(editorRoot, {
        theme: 'snow',
        placeholder: editorRoot.dataset.placeholder || 'Nhập mô tả lookbook...',
        modules: {
            toolbar: {
        ...
        reader.onload = () => {
            ...
            requestAnimationFrame(() => {
                ...
                if (imgNode) {
                    ...
                    imgNode.alt = file.name || 'Ảnh mô tả lookbook';
                }
                syncEditorValue();
            });
        };
    ...
    function insertVideo(file) {
        ...
        quill.insertEmbed(range.index, 'lookbookVideo', { src: previewUrl, token }, 'user');
    ...
    function serializeEditorHtml() {
        ...
        clone.querySelectorAll('[data-upload-token]').forEach((node) => {
            const token = String(node.dataset.uploadToken || '').trim();
            if (!token) return;
            node.setAttribute('src', `${LOOKBOOK_MEDIA_TOKEN_PREFIX}${token}`);
