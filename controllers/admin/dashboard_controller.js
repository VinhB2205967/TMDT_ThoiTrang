...
const { askAI } = require('../../services/aiChat.service');
...
module.exports.trangAITroLy = async (req, res) => {
    return res.render('admin/pages/dashboard/ai_assistant.pug', {
...
module.exports.hoiTroAI = async (req, res) => {
    try {
        ...
        const provider = String(req.body && req.body.provider ? req.body.provider : 'ollama').trim().toLowerCase() || 'ollama';
        ...
        const context = await buildAdminDataContext(question);
        ...
        return res.json({
            ...
            data: {
                ...
                provider: ai && ai.provider ? ai.provider : provider,
                model: ai && ai.model ? ai.model : model,
                contextMeta: {
        ...
        return res.status(500).json({ success: false, message: 'Không thể xử lý câu hỏi AI lúc này.' });
    }
};