...
async function buildCostMap() {
  ...
  const map = new Map();
  rows.forEach((row) => {
    const sanphamid = row._id.sanphamid ? String(row._id.sanphamid) : '';
    const bientheid = row._id.bientheid ? String(row._id.bientheid) : 'main';
    const kichco = row._id.kichco ? String(row._id.kichco) : 'nosize';
    map.set(`${sanphamid}|${bientheid}|${kichco}`, Number(row.avgCost || 0));
  });
  return map;
}
...
function buildItemPipeline(filters, range, options = {}) {
...
async function sumRevenueForRangeItems(filters, range) {
  const rows = await Chitietdonhang.aggregate(buildItemPipeline(filters, range));
...
module.exports.trangBaoCao = async (req, res) => {
  ...
  res.render('admin/pages/reports/index.pug', {
...
module.exports.duLieuBaoCao = async (req, res) => {
  try {
    const status = normalizeStatus(req.query.status || DEFAULT_STATUS);
    const category = String(req.query.category || '').trim();
    const range = buildDateRange(req.query);
    const groupBy = resolveGroupBy(req.query.groupBy, range);
    ...
    if (orderIds.length) {
      ...
      items = await Chitietdonhang.aggregate(itemPipeline);
    }
